(function(m,n){"object"===typeof exports&&exports?n(exports):"function"===typeof define&&define.amd?define(n({})):m.Mustache=n({})})(this,function(m){function n(a){return"function"===typeof a}function w(a){return a.replace(/[\-\[\]{}()*+?.,\\\^$|#\s]/g,"\\$&")}function A(a,c){function b(a){"string"===typeof a&&(a=a.split(B,2));if(!x(a)||2!==a.length)throw Error("Invalid tags: "+a);f=new RegExp(w(a[0])+"\\s*");t=new RegExp("\\s*"+w(a[1]));p=new RegExp("\\s*"+w("}"+a[1]))}if(!a)return[];var d=[],g=
[],e=[],n=!1,l=!1,f,t,p;b(c||m.tags);for(var k=new v(a),q,h,r,u;!k.eos();){q=k.pos;if(r=k.scanUntil(f)){u=0;for(var s=r.length;u<s;++u)if(h=r.charAt(u),C.call(D,h)?l=!0:e.push(g.length),g.push(["text",h,q,q+1]),q+=1,"\n"===h){if(n&&!l)for(;e.length;)delete g[e.pop()];else e=[];l=n=!1}}if(!k.scan(f))break;n=!0;h=k.scan(E)||"name";k.scan(F);"="===h?(r=k.scanUntil(z),k.scan(z),k.scanUntil(t)):"{"===h?(r=k.scanUntil(p),k.scan(G),k.scanUntil(t),h="&"):r=k.scanUntil(t);if(!k.scan(t))throw Error("Unclosed tag at "+
...
m.clearCache=function(){return y.clearCache()};m.parse=function(a,c){return y.parse(a,c)};m.render=function(a,c,b){return y.render(a,c,b)};m.to_html=function(a,c,b,d){a=m.render(a,c,b);if(n(d))d(a);else return a};m.escape=function(a){return String(a).replace(/[&<>"'\/]/g,function(a){return K[a]})};m.Scanner=v;m.Context=p;m.Writer=s;return m});